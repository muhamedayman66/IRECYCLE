...
class HypermarketsScreen extends StatefulWidget {
  ...
  @override
  _HypermarketsScreenState createState() => _HypermarketsScreenState();
}
  ...
  Future<void> fetchStores() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var store in storesData) {
          if (store['category']?.toLowerCase() == 'hypermarkets') {
            ...
            switch (store['name'].toLowerCase()) {
              case 'al rayah market': // عدلناها من 'al-rayah' لـ 'al rayah market'
                imagePath = "assets/images/alrayahHM.jpg";
                break;
              case 'kheir zaman':
                imagePath = "assets/images/kheirzamanHM.jpg";
                break;
              case 'awlad goda': // عدلناها من 'Awlad Goda' لـ 'awlad goda'
                imagePath = "assets/images/godaHM.jpg";
                break;
            }
            hypermarkets.add({
              "name": store['name'],
        ...
        setState(() {
          allStores = hypermarkets;
          filteredStores = hypermarkets;
          isLoading = false;
        });

        if (hypermarkets.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No hypermarkets available at the moment'),
              ),
            );
          }
        }
                                            ...
                                            Expanded(
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Text(
                                                    "Hypermarket, Supermarket, Grocery",
                                                    style: TextStyle(
